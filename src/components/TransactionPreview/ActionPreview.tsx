...
const TransactionStatus = ({
  ...
  const previewBatchTransaction = async (approvalTransaction: boolean = false) => {
    ...
    if (crossChainAction.type === TRANSACTION_BLOCK_TYPE.GMX_STAKE && !approvalTransaction) {
      setIsGettingExplorerLink(false);
      previewTransaction(crossChainAction.transactionHash, approvalTransaction);
      return;
    }
...
const ActionPreview = ({
  ...
  const cost = useMemo(() => {
    ...
    if (!estimated || !estimated?.gasCost) {
      if ((crossChainAction.type === TRANSACTION_BLOCK_TYPE.KLIMA_STAKE || crossChainAction.type === TRANSACTION_BLOCK_TYPE.GMX_STAKE) && crossChainAction.useWeb3Provider && crossChainAction.gasCost) {
  ...
  const additionalTopButtons = [
	...
	if (type === TRANSACTION_BLOCK_TYPE.GMX_STAKE) {
		const { fromAsset, fromChainId, toAsset, providerName, providerIconUrl, receiverAddress } = preview;

		const fromNetwork = supportedChains.find((supportedChain) => supportedChain.chainId === fromChainId);

		const toNetwork = supportedChains[6];

		const toChainTitle = toNetwork?.title ?? CHAIN_ID_TO_NETWORK_NAME[CHAIN_ID.ARBITRUM].toUpperCase();
