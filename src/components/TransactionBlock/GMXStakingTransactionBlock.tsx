...
// Types
import { IGmxStakingTransactionBlock } from '../../types/transactionBlock';
...
import { Chain, CHAIN_ID, supportedChains, gmxAsset } from '../../utils/chain';
...
export interface IGmxStakingTransactionBlockValues {
  fromChainId?: number;
...
const GmxStakingTransactionBlock = ({
  ...
  const [routeToGmx, setRouteToGmx] = useState<BridgingQuote[]>([]);
  ...
  const targetAssetPriceUsd = useAssetPriceUsd(gmxAsset.chainId, gmxAsset.address);
  ...
  useEffect(() => {
    ...
    if (routeToUSDC.length == 0 || routeToGmx.length == 0 || isRouteFetching || !toolUsed) {
      setTransactionBlockFieldValidationError(
        transactionBlockId,
        'route',
  ...
  const computeReceiveAmount = useCallback(debounce(async () => {
    ...
    try {
      ...
      const routeToGmx = await sdk.getCrossChainQuotes({
      ...
      if (routeToGmx.items.length > 0) {
        setSelectedRoute(mapRouteToOption(routeToGmx.items[0]));
        setToolUsed(routeToUsdc.items[0].LiFiBridgeUsed ?? '');
        setRouteToGmx(routeToGmx.items);
        setReceiveAmount(ethers.utils.formatUnits(routeToGmx.items[0].estimate.toAmount, gmxAsset.decimals));
        resetTransactionBlockFieldValidationError(transactionBlockId, 'route');
        setIsRouteFetching(false);
      } else {
        resetRoutes();
  ...
  return (
    <>
      ...
      <NetworkAssetSelectInput
        ...
        hideChainIds={[CHAIN_ID.ARBITRUM]}
...
export default GmxStakingTransactionBlock;
