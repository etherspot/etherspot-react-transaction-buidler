...
import { useEtherspotPrime, useTransactionBuilderModal } from '../hooks';
...
const TransactionsPrimeDispatcherContextProvider = ({ children }: { children: ReactNode }) => {
  const context = useContext(TransactionsDispatcherContext);

  if (context !== null) {
    throw new Error('<TransactionsPrimeDispatcherContextProvider /> has already been declared.');
  }
  ...
  const { getSdkForChainId, web3Provider, providerAddress, accountAddress } = useEtherspotPrime();
    ...
    const processDispatchedCrossChainActions = async () => {
      ...
      await Promise.all(
        unsentCrossChainActions.map(async (unsentCrossChainAction) => {
          ...
          const sdkForChain = await getSdkForChainId(targetChainId);
          ...
          const result: {
            ...
            : await submitEtherspotTransactionsBatch(
                (await getSdkForChainId(unsentCrossChainAction.chainId)) as Sdk,
                transactionsToSend,
      ...
      storedGroupedCrossChainActionsIds.map(async (id) => {
        updatedStoredGroupedCrossChainActions[id] = await Promise.all(
          storedGroupedCrossChainActions[id].map(async (storedCrossChainAction) => {
            const sdkForChain = await getSdkForChainId(storedCrossChainAction.chainId);
            const { batchHash, transactions } = storedCrossChainAction;
            ...
            if (batchHash) {
              try {
                const submittedBatch = await sdkForChain.getUserOpReceipt(batchHash);
      ...
      const crossChainActionsToRestore: ICrossChainAction[] = await Promise.all(
        updatedStoredGroupedCrossChainActions[oldestGroupedCrossChainActionsId].map(async (crossChainAction) => {
          ...
          const estimated = await estimateCrossChainAction(
            await getSdkForChainId(crossChainAction.chainId),
            web3Provider,
            crossChainAction,
            providerAddress,
            accountAddress
  ...
  useEffect(() => {
    const sdkSubscriptions: Subscription[] = [];
    ...
    validPendingCrossChainActionsWithBatches.map(async (crossChainAction) => {
      const sdkForChain = await getSdkForChainId(crossChainAction.chainId);
      if (!sdkForChain) return;
          ...
          .pipe(
            rxjsMap(async (notification) => {
              if (notification?.type === NotificationTypes.GatewayBatchUpdated) {
                const submittedBatch = await sdkForChain.getUserOpReceipt(crossChainAction.batchHash as string);
...
export default TransactionsPrimeDispatcherContextProvider;
