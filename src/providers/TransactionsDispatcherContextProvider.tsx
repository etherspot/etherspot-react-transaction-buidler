...
const TransactionsDispatcherContextProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  ...
  if (context !== null) {
    throw new Error(
      '<TransactionsDispatcherContextProvider /> has already been declared.'
    );
  }

  const [processingCrossChainActionId, setProcessingCrossChainActionId] =
  ...
  const { getSdkForChainId, web3Provider, providerAddress, accountAddress } =
    useEtherspot();
  ...
  const dispatchCrossChainActions = useCallback(
    (crossChainActionsToDispatch: ICrossChainAction[], status?: string) => {
      ...
      const updatedCrossChainActionsToDispatch =
        updateCrossChainActionsTransactionsStatus(
  ...
  const updatedStoredCrossChainActions = useCallback(
    (
      dispatchIdToUpdate: string,
      crossChainActionsToUpdate: ICrossChainAction[]
    ) => {
      try {
        const storedGroupedCrossChainActionsRaw = getItem(
          STORED_GROUPED_CROSS_CHAIN_ACTIONS
        );
        let storedGroupedCrossChainActions = {};
        if (storedGroupedCrossChainActionsRaw)
          storedGroupedCrossChainActions = JSON.parse(
            storedGroupedCrossChainActionsRaw
          );
        setItem(
          STORED_GROUPED_CROSS_CHAIN_ACTIONS,
          JSON.stringify({
  ...
  const resetCrossChainActions = useCallback(
    (errorMessage?: string) => {
      if (dispatchId && crossChainActions?.length) {
        const updatedCrossChainActions =
          rejectUnsentCrossChainActionsTransactions(crossChainActions);
  ...
  const processDispatchedCrossChainActions = useCallback(async () => {
    ...
    const pendingCrossChainActionTransactions =
      getCrossChainActionTransactionsByStatus(
        firstUnsentCrossChainAction.transactions,
    ...
    if (
      !unsentCrossChainActionTransactions?.length ||
      hasUnsentAndPendingWeb3ProviderTransactions
    )
      return;
    ...
    const updatedCrossChainActions = crossChainActions.map(
      (crossChainAction) => {
        if (
          crossChainAction.id !== firstUnsentCrossChainAction.id &&
          (!crossChainAction.multiCallData?.id ||
            firstUnsentCrossChainAction.multiCallData?.id !==
              crossChainAction.multiCallData.id)
        ) {
        ...
        const updatedTransactions = crossChainAction.transactions.reduce(
          (updated: ICrossChainActionTransaction[], transaction) => {
            ...
            if (
              ...
              (isUnsentTransaction && !isUnsentTransactionUpdated)
            ) {
  ...
  const restoreProcessing = useCallback(async () => {
    ...
    const updatedStoredGroupedCrossChainActions = {
      ...storedGroupedCrossChainActions,
    };

    // newest to oldest
    const storedGroupedCrossChainActionsIds = Object.keys(
      storedGroupedCrossChainActions
    )
      .sort()
      .reverse();
    ...
    await Promise.all(
      storedGroupedCrossChainActionsIds.map(async (id) => {
        updatedStoredGroupedCrossChainActions[id] = await Promise.all(
          storedGroupedCrossChainActions[id].map(
            async (storedCrossChainAction) => {
              const sdkForChain = getSdkForChainId(
                storedCrossChainAction.chainId
              );
              ...
              if (batchHash) {
                try {
                  const submittedBatch =
                    await sdkForChain.getGatewaySubmittedBatch({
                      hash: batchHash,
                    });
    ...
    const oldestGroupedCrossChainActionsId =
      storedGroupedCrossChainActionsIds.find(
        (id) =>
          !!getFirstCrossChainActionByStatus(
    ...
    if (oldestGroupedCrossChainActionsId) {
      ...
      const crossChainActionsToRestore: ICrossChainAction[] = await Promise.all(
        updatedStoredGroupedCrossChainActions[
          oldestGroupedCrossChainActionsId
        ].map(async (crossChainAction) => {
    ...
    setItem(
      STORED_GROUPED_CROSS_CHAIN_ACTIONS,
      JSON.stringify(updatedStoredGroupedCrossChainActions)
    );

    const hasPending = storedGroupedCrossChainActionsIds.some((id) =>
      storedGroupedCrossChainActions[id].some(
        async (storedCrossChainAction) => {
        ...
        subscription = sdkForChain.notifications$
          .pipe(
            rxjsMap(async (notification) => {
              if (
                notification?.type === NotificationTypes.GatewayBatchUpdated
              ) {
                const submittedBatch =
                  await sdkForChain.getGatewaySubmittedBatch({
                ...
                setCrossChainActions((current) =>
                  current.map((currentCrossChainAction) => {
                    ...
                    const updatedTransactions =
                      currentCrossChainAction.transactions.map(
                        (transaction) => ({
                          ...transaction,
                          finishTimestamp: updatedStatus
                            ? +new Date()
                            : undefined,
