...
const TransactionBuilderContextProvider = ({
  ...
  const formUrlOptions = (options: { [key: string]: string }): string => {
    ...
    Object.keys(options).map((key: string) => {
      ...
      optionStr += `${!optionStr ? '?' : '&'}${key}=${encodeURIComponent(
        value
      )}`;
    });
    return optionStr;
    ...
    <TransactionBuilderContext.Provider value={{ data: contextData }}>
      <TopNavigation>
        <WalletAddressesWrapper onClick={hideMenu}>
          ...
          {accountAddress && (
            <WalletAddress onClick={onRampClick}>Buy</WalletAddress>
          )}
          {!accountAddress && (
            ...
            <>
              {crossChainActions.map((crossChainAction) => (
                <ActionPreview
                  ...
                  signButtonDisabled={
                    crossChainAction.isEstimating || isSigningAction
                  }
                  ...
                  onSign={async () => {
                    ...
                    if (
                      ...
                      showAlertModal(
                        result.errorMessage ?? 'Unable to send transaction!'
                      );
                      return;
                    }
