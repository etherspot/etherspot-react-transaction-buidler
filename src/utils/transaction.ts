...
export const klimaDaoStaking = async (
  ...
  errorMessage?: string;
  result?: { transactions: ICrossChainActionTransaction[]; provider?: string };
}> => {
  ...
  try {
    ...
    if (
      !addressesEqual(
        fromAssetAddress,
        nativeAssetPerChainId[CHAIN_ID.POLYGON].address
      ) &&
    ...
    if (
      !klimaApprovalTransactionRequest ||
      !klimaApprovalTransactionRequest.to
    ) {
      return { errorMessage: 'Failed build bridge approval transaction!' };
    }
    ...
    const klimaStakeTransactionRequest = klimaStakingContract.encodeStake?.(
      bestRoute.estimate.toAmount
    ); // Klima staking
    ...
    transactions = [
      ...transactions,
      klimaApprovalTransaction,
      klimaStakinglTransaction,
    ];
...
export const buildCrossChainAction = async (
      ...
      if (fromChainId !== CHAIN_ID.POLYGON) {
        try {
          ...
          if (
            ...
            !addressesEqual(
              fromAssetAddress,
              nativeAssetPerChainId[fromChainId].address
            ) &&
            ...
            if (!approvalTransactionRequest || !approvalTransactionRequest.to) {
              return {
                errorMessage: 'Failed build bridge approval transaction!',
              };
            }
          ...
          const result = await klimaDaoStaking(
            routeToKlima,
            receiverAddress,
            sdk,
            true,
          ...
          const crossChainAction: ICrossChainAction = {
            ...
            receiveAmount: ethers.utils
              .parseUnits(routeToUSDC.estimate.toAmount ?? '0', 6)
              .toString(),
        ...
        return {
          errorMessage: 'Failed to fetch any offers for this asset to USDC',
        };
      }
    } catch (e) {
  ...
  if (
    ...
    try {
      ...
      const bridgeServiceDetails =
        bridgeServiceIdToDetails[fistStep?.toolDetails?.key ?? ''];

      const preview = {
        fromChainId,
        toChainId,
        providerName:
          fistStep?.toolDetails?.name ?? bridgeServiceDetails?.title ?? 'LiFi',
        providerIconUrl:
          fistStep?.toolDetails?.logoURI ?? bridgeServiceDetails?.iconUrl,
        fromAsset: {
      ...
      const { items: advancedRouteSteps } = await sdk.getStepTransaction({
        route,
      });
  ...
  if (
    ...
    try {
      ...
      let transactions: ICrossChainActionTransaction[] = offer.transactions.map(
        (transaction) => ({
          ...transaction,
          chainId,
          createTimestamp,
      ...
      if (
        ...
        !addressesEqual(
          fromAssetAddress,
          nativeAssetPerChainId[chainId].address
        ) &&
        transactions.length === 1
      ) {
        const abi = getContractAbi(ContractNames.ERC20Token);
...
export const submitEtherspotTransactionsBatch = async (
  ...
  try {
    if (
      !sdk?.state?.account?.type ||
      sdk.state.account.type === AccountTypes.Key
    ) {
    ...
    const feeToken = isZeroAddress(feeTokenAddress)
      ? undefined
      : feeTokenAddress;
          ...
          rxjsMap(async (notification) => {
            if (notification.type === NotificationTypes.GatewayBatchUpdated) {
              ...
              if (
                ...
                finishSubscription = () =>
                  resolve({ errorMessage: 'Failed Transaction sent' });
              } else if (submittedBatch?.transaction?.hash) {
                finishSubscription = () =>
                  resolve({ transactionHash: submittedBatch.transaction.hash });
              }

              if (finishSubscription) {
                if (temporaryBatchSubscription)
                  temporaryBatchSubscription.unsubscribe();
                finishSubscription();
              }
            }
          })
...
export const submitWeb3ProviderTransactions = async (
  ...
  if (!changed)
    return { errorMessage: 'Unable to change to selected network!' };
  ...
  try {
    for (const transaction of transactions) {
      ...
      // @ts-ignore
      transactionHash = await web3Provider.sendRequest('eth_sendTransaction', [
        tx,
      ]);
    }
  } catch (e) {
...
export const estimateCrossChainAction = async (
  ...
  try {
    const balancesForAddress =
      crossChainAction.useWeb3Provider && providerAddress
        ? providerAddress
        : accountAddress;
    ...
    crossChainAction.transactions.map((transactionsToSend) => {
      ...
      if (
        ...
        !addressesEqual(crossChainAction.gasTokenAddress, outgoingAsset.address)
      )
        return;
  ...
  if (crossChainAction.useWeb3Provider) {
    ...
    try {
      ...
      const { estimation: gatewayBatchEstimation } =
        await sdk.estimateGatewayBatch({ feeToken });
      gasCost = gatewayBatchEstimation.estimatedGasPrice.mul(
        gatewayBatchEstimation.estimatedGas
      );
...
export const getTransactionExplorerLink = (
  ...
  const explorerUrl = supportedChains.find(
...
// 0,10 – first 4 bytes
const ERC20ApprovalMethodId = ethers.utils
  .keccak256(ethers.utils.toUtf8Bytes('approve(address,uint256)'))
  .slice(0, 10);

export const isERC20ApprovalTransactionData = (
...
export const getFirstCrossChainActionByStatus = (
  ...
  crossChainActions.find(({ transactions }) =>
    transactions.find((transaction) => transaction.status === status)
  );
...
export const filterCrossChainActionsByStatus = (
  ...
  crossChainActions.filter(({ transactions }) =>
    transactions.find((transaction) => transaction.status === status)
  );
...
export const getCrossChainActionTransactionsByStatus = (
  ...
  crossChainActionTransactions.filter(
...
export const rejectUnsentCrossChainActionsTransactions = (
  ...
  crossChainActions.map((crossChainActionToDispatch) => ({
    ...
    transactions: crossChainActionToDispatch.transactions.map((transaction) => {
      if (transaction.status !== CROSS_CHAIN_ACTION_STATUS.UNSENT)
        return transaction;
      return {
        ...transaction,
